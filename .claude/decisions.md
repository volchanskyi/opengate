...
| 025 | CD pre-flight digest check short-circuits a staging deploy when the target digest and `deploy/**` are both unchanged | — | Accepted | [ADR-025](../docs/adr/ADR-025-cd-preflight-digest-check.md) |
...
| 084 | Staging's browser suite gets two real machines it cross-builds and enrols itself; certificate named for the in-cluster server, operator registered first, load-test administrator reseeded last | Staging fleet | Accepted | [ADR-084](../docs/adr/ADR-084-staging-e2e-runs-against-real-machines.md) |
