...
MANDATORY RULES (enforced by .claude/hooks/, NO bypass):
  ...
  - Every commit attempt runs scripts/precommit-gauntlet.sh — no marker, no bypass (git-commit-guard)
  - Run /refactor after a commit lands, before pushing; marker validates via git rev-parse HEAD (git-push-guard)
