...
| Alerts reach the queue | The join between the two halves: the machine's queue drained on its heartbeat and re-offered after a failed send, the rules it owns registered centrally, and a rule change reaching machines already connected. [ADR-124](../docs/adr/ADR-124-an-alert-reaches-the-queue.md). |
