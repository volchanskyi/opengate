...
| File | Change |
|---|---|
| [`scripts/tests/docs-diagrams.test.sh`](../../../scripts/tests/docs-diagrams.test.sh) | Add `assert_mermaid_count_at_least` for [`docs/Wire-Protocol.md`](../../../docs/architecture/Wire-Protocol.md) (≥1), [`docs/Monitoring.md`](../../../docs/infrastructure/Monitoring.md) (≥1), [`docs/adr/ADR-025-cd-preflight-digest-check.md`](../../../docs/adr/ADR-025-cd-preflight-digest-check.md) (≥1); convert the inline Multiscale check to the helper; add a **total-fence floor** (`grep -rc '^```mermaid$' docs` ≥ current count). Keep all existing assertions + the blob/fence bans. |
