...
**Branch:** `dev`. **Owner:** engineer (k8s + scripts). **Sequence:** after FI4. **Depends on:** a green staging deploy + the FI1 recovery-SLO budgets ([`docs/Fault-Injection.md`](../../../docs/infrastructure/Fault-Injection.md)). (FI3 is obsolete — no server-side fault config; C1 is a direct `kubectl delete` runner, not Chaos Mesh PodChaos, so no controller is needed; SLOs use FI1's initial-generous budgets, not yet-unmeasured p95.)
