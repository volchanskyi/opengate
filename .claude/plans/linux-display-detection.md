...
OpenGate has grown through 11 phases — Rust agent, Go server, React web client, WebRTC, Intel AMT MPS, admin dashboard, CD pipeline — but comprehensive end-to-end testing is still missing. The codebase has 191 Go tests (13 integration), 66 Rust tests, and ~35 web test files, but no Playwright browser tests, no load testing, and a known flaky test. Phase 12 closes this gap before the project scales further. Two new audit skills (`/infra-audit`, `/backend-audit`) also need workflow integration.

---

## Workstream 1: Fix Flaky Test (first)

**File:** [session_test.go:345](server/tests/integration/session_test.go#L345)
...
### New test helpers in [testutil.go](server/internal/testutil/testutil.go)
...
### Add to existing [session_test.go](server/tests/integration/session_test.go)
...
- **No real agent** — device data seeded via test-only endpoint or DB
- **`tmpfs: /data`** — ephemeral database, each `docker compose down/up` = clean slate
- **`OPENGATE_TEST_MODE=true`** — enables test seed endpoint
...
For true QUIC agent load testing, a custom Go binary:
...
## Workstream 7: CI Pipeline Updates

### Changes to [ci.yml](.github/workflows/ci.yml)
...
- Add `e2e` to `needs` list (line 568)

**Update `notify-failure`:**
- Add `e2e` to `needs` list (line 661)
...
- Uses a separate `playwright.staging.config.ts` with `baseURL: 'http://127.0.0.1:18080'`
...
## Future Follow-Up: Full Agent E2E Testing

This section outlines what's needed to run true end-to-end tests with a **real Rust agent connecting to the server**. This becomes possible once Phase 12's Docker Compose test infra is in place and a runnable agent binary is created.
...
### When to Execute
After Phase 14 (Agent Auto-Update) when the agent binary is fully functional with auto-update support. The agent binary is a prerequisite — without it, this plan is blocked.
...
4. **Playwright E2E:** `make e2e` — all ~20 specs pass
