...
| Next session-protocol change | Rust agent | ADR-024 | ✅ `MouseHandler` carved 2026-05-29 — owns `MouseMove` + `MouseClick` in `agent/crates/mesh-agent-core/src/session/handlers/mouse.rs`; `ControlMessageHandler` marker trait in `handlers/mod.rs`; integration test in `tests/mouse_handler_test.rs` + 11 unit tests inline. ⏳ Remaining: `KeyboardHandler` (KeyPress), `FileHandler` (FileList + FileDownload + FileUpload), `WebRTCHandler` (SwitchToWebRTC + IceCandidate), `SwitchHandler` (SwitchAck), `TerminalControlHandler` (TerminalResize) — added opportunistically as `handle_control` arms are touched. |
