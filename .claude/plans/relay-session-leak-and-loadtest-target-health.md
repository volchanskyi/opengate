...
**Status:** ready to start. **Branch:** `dev`. **Owner:** agent (Go, Helm, CI).
...
**Files:** [`server/internal/signaling/tracker.go`](../../server/internal/signaling/tracker.go),
[`server/internal/api/api.go`](../../server/internal/api/api.go),
...
```
WS5.1 (process_start_time alert)   ── land first; smallest, catches a recurrence tonight
WS1  (the fix, + D5)               ── second; its slope test lands before the source change
WS8  (the gate and the rule)       ── third; generalises WS1's test while the reasoning is fresh
WS2  (liveness + teardown order)   ── fourth; builds on WS1's parked handler
WS4  (the run's conservation record)
WS3, WS5.2-5, WS6, WS7             ── independent, any order
```

WS8 lands third on purpose. Its test is WS1's test generalised, so writing it
while that reasoning is still in hand costs a fraction of what it costs later —
and until it exists, nothing stops the same shape appearing in the next
long-lived connection path.

**Interim mitigation, and its expiry.** Restarting the staging server at the
start of each load run would make the symptom go away today. Do it only as a
stopgap and only with WS5.1 already landed — without the alert it removes the
signal rather than the fault. Do **not** raise the 384Mi limit: an unbounded
leak defeats any ceiling, and a larger one only lengthens the fuse. Delete the
stopgap in the commit that lands WS1.
...
| Artefact | Content |
|---|---|
| **ADR-093** | A relay session's lifetime is owned by the relay, not by a hijacked request context, and its peers are proved alive rather than assumed. Context: `websocket.Accept` hijacks, so the request context is never cancelled — not by a client hangup and not by `Server.Shutdown`, because a hijacked connection is untracked. The happy path leaked and the failure path did not. Record both rejected alternatives: deleting the wait (WS1) and a hard session-duration cap (WS2). |
| **ADR-094** | A load run records what its target was holding: a run whose target was replaced mid-run is invalid, not failed, and a run whose target did not give back what it took is failed. The sibling of ADR-090, and the process-side counterpart of `CleanupProof`. |
| **ADR-095** | The server has two listeners: what the world may reach, and what only the cluster may. Context: SPA serving moved into the binary and collapsed three edge routes into one catch-all, so the boundary two comments asserted had stopped existing. |
| **ADR-096** | A counter of a resource is not a measurement of it. Context: every liveness number the server published returned to zero correctly while the process held 7,455 goroutines, because each is maintained by the teardown path rather than read from the resource. The sibling of ADR-091's read-back doctrine and ADR-088's *a gate measures the system, not its own harness*. |
| [`decisions.md`](../decisions.md) | one row each, ≤200 characters of prose |
| [`phases.md`](../phases.md) | Completed row on landing, linking `plans/archive/…` |
...
refuses a Completed row pointing at a non-archived plan.

---

## 7. Reviewer checklist

- [ ] A failing test preceded every source change (branch history shows it).
- [ ] `registerAndWait` waits on the session's own done channel and a
      **server-lifetime** context — never on `r.Context()` — and closes its
      websocket with `CloseNow` on the way out.
- [ ] The session done channel is closed exactly once on **both** teardown
      paths — `pipe`'s defer and `Unregister`.
- [ ] The leak test is a **slope** across several session counts, not a fixed
      `NumGoroutine()` baseline, and it fails on the pre-fix code.
- [ ] `Shutdown` ends live relay sessions instead of returning immediately.
- [ ] A relay write carries a deadline; a stalled peer ends the session within a
      stated ping budget; no hard session-duration cap was added.
- [ ] `pipe`'s teardown updates its books **before** it waits on the network,
      matching `Unregister`, and closes the two sides concurrently.
- [ ] `holdOpen` fails on a peer that dies mid-hold.
- [ ] A mid-run target restart classifies **invalid**; a target that did not
...
- [ ] The four `/metrics` call sites moved together, and the three previously
      unguarded ones each gained a gate in the same commit.
- [ ] The smoke test's `--domain` branch **asserts the boundary** instead of
      skipping, and CD runs it.
- [ ] pprof is absent from the public listener; the pen-test gate agrees.
- [ ] The cAdvisor job, the ClusterRole grant, and all three alerts each have a
      gate landing in the same commit.
- [ ] No map in the process has an eviction path with no caller; `auditLog`
      holds a bound and counts what it sheds.
- [ ] The conservation test lives in the integration tier, fails on the pre-fix
      code, and states its tolerance and why.
- [ ] The semgrep rule ships with **both** fixtures — one it fires on, one it is
      silent on.
- [ ] `.claude/rules/resource-conservation.md` exists, names what enforces it,
      and has its row in the `CLAUDE.md` index.
- [ ] No new non-test `.go` file under `server/` without its
      [mutation-shards.sh](../../scripts/lib/mutation-shards.sh) entry
      (`tests/loadtest` is already a `dir:` shard; `internal/app` is not — check).
- [ ] Any new `scripts/tests/*.test.sh` is `100755`.
- [ ] The stopgap restart, if it was added, is gone.
- [ ] No doc or comment narrates the removed behaviour — say what the code does
