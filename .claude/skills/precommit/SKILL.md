...
The gauntlet does not assert documentation freshness. After it passes, before committing, update [`README.md`](../../../README.md) sections and [`/docs`](../../../docs/) pages that the diff invalidates (link-over-paraphrase + live-state ADR conventions per [`docs/README.md`](../../../docs/README.md)). ADRs in [`docs/adr/`](../../../docs/adr/) describe live state — edit one in place when its decision changes, delete it when the decision leaves nothing behind, and fold a minor fix into the ADR it refines.
