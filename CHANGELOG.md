# Changelog

All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.1.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## [v0.107.0] - 2026-09-26

### Added
- **alerts:** the alert nobody received, and the switch that did nothing

### Fixed
- **ci,alerts:** four red nights, and messages worth reading

## [v0.106.1] - 2026-09-23

### Fixed
- **ci,shell:** the input three legs never named, and the guard that lost it

## [v0.106.0] - 2026-09-23

### Added
- **ci:** the alert nobody heard, and four gates beside it

## [v0.105.0] - 2026-09-22

### Added
- **loadtest:** three readings a night could not take

## [v0.104.0] - 2026-09-17

### Added
- **loadtest:** open the sessions the soak measures

## [v0.103.5] - 2026-09-16

### Fixed
- **loadtest:** at a venue's ceiling, hold the middle case

## [v0.103.4] - 2026-09-16

### Fixed
- **loadtest:** a limit is bracketed by nights of its own load

## [v0.103.3] - 2026-09-16

### Fixed
- **loadtest,metrics:** the third delay, waited out

## [v0.103.2] - 2026-09-16

### Fixed
- **tooling:** an install names its version, wherever it is written
- **loadtest:** a machine is counted while it is attached

## [v0.103.1] - 2026-09-16

### Fixed
- **loadtest,metrics:** one fleet, one instant

## [v0.103.0] - 2026-09-15

### Added
- **mutation,loadtest:** per-shard walk, venue ceiling

## [v0.102.0] - 2026-09-14

### Added
- **loadtest:** three register entries, paid down

## [v0.101.4] - 2026-09-14

### Fixed
- **loadtest:** count the machines that arrived, not the survivors

## [v0.101.3] - 2026-09-14

### Fixed
- **loadtest,ci:** three faults, one per family

## [v0.101.2] - 2026-09-11

### Fixed
- **loadtest:** a machine the run stood down did not fail to arrive

## [v0.101.1] - 2026-09-11

### Fixed
- **loadtest:** file a machine the server has finished registering

## [v0.101.0] - 2026-09-11

### Added
- **loadtest:** see inside the leak

## [v0.100.0] - 2026-09-11

### Added
- **loadtest:** offer the profile's load, one address per technician

## [v0.99.1] - 2026-09-10

### Fixed
- **loadtest:** offer the load the profile declares, on a node with room

## [v0.99.0] - 2026-09-10

### Added
- **loadtest:** file the estate as it arrives

## [v0.98.1] - 2026-09-10

### Fixed
- **loadtest:** a machine leaves when the run says so

## [v0.98.0] - 2026-09-10

### Added
- **loadtest:** give the sweeps a consumer, and something to read

## [v0.97.2] - 2026-09-10

### Fixed
- **loadtest:** read a busy machine by its venue

## [v0.97.1] - 2026-09-10

### Fixed
- **loadtest:** give the in-pod harness a path the pod has

## [v0.97.0] - 2026-09-10

### Added
- **loadtest:** repair the venue, schedule the families

## [v0.96.3] - 2026-09-09

### Fixed
- **loadtest,ci:** four nightlies that could not measure themselves

### Changed
- **loadtest:** name the arrival signal, and count it in one place

## [v0.96.2] - 2026-09-07

### Fixed
- **ci:** one pinned version, one honest assertion
- **fault:** a drill measures a herd that is there

## [v0.96.1] - 2026-09-07

### Fixed
- **fault:** a drill reading is the scenario's own

## [v0.96.0] - 2026-09-07

### Added
- **loadtest:** one measurement, one limit, one file

## [v0.95.0] - 2026-09-06

### Added
- **loadtest:** a bundle field is a reading or it is absent

## [v0.94.4] - 2026-09-06

### Fixed
- **loadtest,server:** three flakes the nightly multiplies by twenty-seven

## [v0.94.3] - 2026-09-05

### Fixed
- **fault:** settle the counts and close without racing

## [v0.94.2] - 2026-09-05

### Fixed
- **ci:** name the fleet pod the shim asks for

## [v0.94.1] - 2026-09-05

### Fixed
- **ci:** spell a gh api read as a read

## [v0.94.0] - 2026-09-04

### Added
- **fault:** drill the machine-facing link nightly

## [v0.93.7] - 2026-09-04

### Fixed
- **load:** hold the QUIC fleet in the cluster, not on a stream

## [v0.93.6] - 2026-09-04

### Fixed
- **ci:** two gates that graded the wrong thing

## [v0.93.5] - 2026-09-04

### Fixed
- **ci,server:** three red gates, none of them about the change under test

## [v0.93.4] - 2026-09-03

### Fixed
- **web,server:** four tests that could not fail, and one guard that let a link out

## [v0.93.3] - 2026-09-03

### Fixed
- **deploy:** ask the exposition for what it always has

## [v0.93.2] - 2026-09-03

### Fixed
- **ci,server:** budget the leash a blocked mutant holds

## [v0.93.1] - 2026-09-02

### Fixed
- **ci,deploy,deps:** point the edge smoke at the edge

## [v0.93.0] - 2026-09-02

### Added
- **loadtest,monitoring,server:** see the target, and keep the exposition off the edge

### Fixed
- **deploy,web,docs:** wrap the compose command, drop a grant, patch fast-uri

## [v0.92.1] - 2026-09-02

### Fixed
- **relay:** prove the peer is alive, and book out before waiting on the network

## [v0.92.0] - 2026-09-02

### Added
- **monitoring:** alert when the server process is replaced

### Fixed
- **relay:** a session's lifetime belongs to the relay, not to a hijacked context

## [v0.91.19] - 2026-09-01

### Fixed
- **loadtest,ci,rules:** size k6 to the rate limit, audit every lockfile

### Changed
- **tests:** one exit path, and a limit the guard reads from the right call

## [v0.91.18] - 2026-09-01

### Fixed
- **loadtest,ci:** five numbers that measured something else

## [v0.91.17] - 2026-08-31

### Fixed
- **ci:** a Rust test file is test code, and the guard that said so was guessing
- **loadtest,ci,agent:** four debts, and the language the gate never saw

## [v0.91.16] - 2026-08-31

### Fixed
- **server,web,ci:** sweep retention, fix two gates

## [v0.91.15] - 2026-08-30

### Fixed
- **agentapi,amt,mutation:** bound the wait for a listener's address

## [v0.91.14] - 2026-08-30

### Fixed
- **agentapi,mutation:** order a device's status writes; provision the metrics store

## [v0.91.13] - 2026-08-29

### Fixed
- **agent,server:** owner-only agent key, TLS resume counter

## [v0.91.12] - 2026-08-29

### Fixed
- **ci,loadtest,perf:** unstick three red workflows
- **ci:** report the log a CD failure issue could not read

### Changed
- **server:** start periodic workers from the assembly

## [v0.91.11] - 2026-08-29

### Fixed
- **web,server,ci:** scope hardware e2e text, settle vm reaper

## [v0.91.10] - 2026-08-29

### Fixed
- **server,web,ci:** four debts paid, and the shard that reported work it never did

## [v0.91.9] - 2026-08-28

### Fixed
- **ci,deploy:** the deploy read a note about the cluster instead of the cluster

## [v0.91.8] - 2026-08-28

### Fixed
- **server:** a package failed on a reaper container it never started

## [v0.91.7] - 2026-08-28

### Fixed
- **ci,deploy:** the deploy built a binary on a cache it can never fill

## [v0.91.6] - 2026-08-28

### Fixed
- **ci,deploy:** a warning became the first line of the lock, and nothing could read it

## [v0.91.5] - 2026-08-28

### Fixed
- **ci,deploy:** a lock wrote a timestamp the API refused, and read the refusal as a holder

## [v0.91.4] - 2026-08-27

### Fixed
- **agent,ci,deploy:** a machine died on its log directory, and nothing kept two runs out of one namespace

## [v0.91.3] - 2026-08-27

### Fixed
- **ci,deploy,web:** a second stack ran the same specs and brought up no machines

## [v0.91.2] - 2026-08-26

### Fixed
- **deploy,server:** a chart hook wrote a column list the schema outgrew

## [v0.91.1] - 2026-08-26

### Fixed
- **ci,loadtest,deploy,docs:** four nightlies that went red, and the tests that would have caught them

## [v0.91.0] - 2026-08-22

### Added
- **loadtest,server,ci,docs:** a night that measures the system, or says it did not

## [v0.90.2] - 2026-08-21

### Fixed
- **ci,deploy,docs:** a stack whose shape validates without its credential

## [v0.90.1] - 2026-08-19

### Fixed
- **ci,edge:** the shards that outgrew their cap, and the programme's last debts

## [v0.90.0] - 2026-08-19

### Added
- **rules:** the front door for curated detection

### Fixed
- **quality:** the twelve the blame gap hid

## [v0.89.0] - 2026-08-17

### Added
- **edge:** the room a person actually opens

## [v0.88.0] - 2026-08-16

### Added
- **edge:** a queue a person can actually work

## [v0.87.0] - 2026-08-15

### Added
- **edge:** watching the pack that watches the fleet

### Fixed
- **quality:** what an exclusion has to earn, and the one that still cannot

## [v0.86.0] - 2026-08-15

### Added
- **edge:** three hundred and twelve rows, and the one room they are

## [v0.85.0] - 2026-08-15

### Added
- **edge:** where an alert lands, and what a purge owes the room it leaves

### Fixed
- **edge:** say the tenant in full, and name a port after what it does

## [v0.84.0] - 2026-08-14

### Added
- **edge:** an alert that arrives carrying everything behind it

## [v0.83.0] - 2026-08-14

### Added
- **edge:** a rule that turns out to be wrong, and the machine that pays for it

## [v0.82.0] - 2026-08-13

### Added
- **edge:** has this happened before, and how far back the device can answer

## [v0.81.0] - 2026-08-13

### Added
- **edge:** where a rule lives, and what a customer may change about it

## [v0.80.0] - 2026-08-13

### Added
- **edge:** what a rule may say, and which machines it is watching

## [v0.79.0] - 2026-08-13

### Added
- **edge:** failures that never cross a line, and correlation on the device

## [v0.78.0] - 2026-08-12

### Added
- **telemetry:** measure how slow a disk is, not just how full

## [v0.77.0] - 2026-08-10

### Added
- **telemetry:** read stalling from the kernel, and never invent it

## [v0.76.0] - 2026-08-10

### Added
- **telemetry:** measure what an active series costs, and stop with the number

## [v0.75.0] - 2026-08-08

### Added
- **telemetry:** the vitals contract — 60 s cadence, extrema, and a bounded dim vocabulary

## [v0.74.3] - 2026-08-08

### Fixed
- **load:** point the k6 scenarios at sites, and stop trending crashed runs

## [v0.74.2] - 2026-08-08

### Fixed
- **agent:** report the fullest mount, not the disk pool average

## [v0.74.1] - 2026-08-07

### Fixed
- **cd:** reset the table the migrations actually leave behind

## [v0.74.0] - 2026-08-07

### Added
- **telemetry:** make a chart span the window it was asked for

## [v0.73.1] - 2026-08-07

### Fixed
- **telemetry:** make every received message either land or say why not

## [v0.73.0] - 2026-08-07

### Added
- **tenancy:** give a customer sites, and settings a ladder to resolve down
- **tenancy:** give a tenant customers, and every fleet read a filter

## [v0.72.6] - 2026-08-07

### Fixed
- **deps:** take rkyv 0.8.17 for the archive-validation advisories

### Changed
- **tenancy:** name the outer ring the tenant
- **agent:** fold the journalctl invocation into one runner
- **agent:** implement Linux only, keep the platform extension points

## [v0.72.5] - 2026-08-03

### Fixed
- **protocol:** close the server-to-agent wire contract, measure the carved-out paths

### Changed
- **tests:** clarify the wire-contract test scaffolding

## [v0.72.4] - 2026-08-03

### Fixed
- **web:** keep relay tokens out of error reports, allowlist link schemes

## [v0.72.3] - 2026-08-03

### Fixed
- **security:** close installer command injection, rate-limit bypass, relay auth

## [v0.72.2] - 2026-07-31

### Fixed
- **load:** drive the k6 scenarios as a member, not as an admin

## [v0.72.1] - 2026-07-31

### Fixed
- **agent:** count allocations on the thread under test, not the process
- **e2e:** make the empty-fleet assertions independent of what ran before

## [v0.72.0] - 2026-07-30

### Added
- **api:** scope reads to the organization and answer the dashboard in O(1)

## [v0.71.1] - 2026-07-30

### Fixed
- **db:** run migrations with the tenant scope forced RLS requires

## [v0.71.0] - 2026-07-29

### Added
- **amt:** make Intel AMT a device property and repair CIRA tenancy

## [v0.70.2] - 2026-07-26

### Fixed
- **session:** repair terminal rendering and cleanup

## [v0.70.1] - 2026-07-26

### Fixed
- **ci,protocol:** repair the nightly quality gates

## [v0.70.0] - 2026-07-26

### Added
- **web,server:** drop redundant refresh controls, collect stale sessions

## [v0.69.0] - 2026-07-25

### Added
- **web,server:** cache device logs, collapse panes, drag-and-drop regrouping

## [v0.68.0] - 2026-07-25

### Added
- **web,agent,server:** device-detail UX + telemetry persist coalescing

## [v0.67.0] - 2026-07-24

### Added
- **web,agent,server:** System Logs pane + on-demand host log collection

## [v0.66.0] - 2026-07-24

### Added
- **agent:** stream live host metrics to central VictoriaMetrics

## [v0.65.0] - 2026-07-23

### Added
- **web:** icon-only device actions, live family utilisation, hardware auto-load

## [v0.64.2] - 2026-07-23

### Fixed
- **web,agent:** declutter device logs UI, collapsible footprint, fleet-health telemetry

## [v0.64.1] - 2026-07-21

### Fixed
- **ci:** scope down go-agentapi-backfill mutation timeout coefficient

## [v0.64.0] - 2026-07-20

### Added
- **web:** maintenance-mode UI — toggle, badges, fleet count, empty states (WS-E)

## [v0.63.0] - 2026-07-18

### Added
- **maintenance:** server-side maintenance mode — toggle, push, fleet count (WS-C)

## [v0.62.0] - 2026-07-18

### Added
- **protocol:** SetMaintenanceMode + MaintenanceApplied wire messages (WS-B)

## [v0.61.0] - 2026-07-18

### Added
- **agent:** Edge-Sentinel collectors always-on; remove opt-in flags (WS-A)

## [v0.60.0] - 2026-07-17

### Added
- **fault:** FI6 wire deployed fault drills into staging CD; close out rollout

## [v0.59.0] - 2026-07-17

### Added
- **fault:** FI5 k8s scenario runner (C1 pod-delete, C2 bad-rollout); fix CI Config Lint
- **fault:** FI4 ingress edge fault profiles; retire obsolete FI3

## [v0.58.0] - 2026-07-16

### Added
- **faulttest:** fault-injection FI1 spec + FI2 Go test-harness suite

## [v0.57.3] - 2026-07-14

### Fixed
- **ci:** isolate mutation timeout shards

## [v0.57.2] - 2026-07-14

### Fixed
- **ci:** rebalance mutation tail shards

## [v0.57.1] - 2026-07-14

### Fixed
- **ci:** recover mutation workflow completeness

## [v0.57.0] - 2026-07-13

### Added
- **lifecycle:** add data lifecycle erasure

### Fixed
- **sonar:** resolve lifecycle quality findings

## [v0.56.0] - 2026-07-02

### Added
- harden multitenant deploy and wire edge sentinel protocol

## [v0.55.0] - 2026-07-01

### Added
- **edge-sentinel:** land WS-0 WS-1 WS-2 foundations

## [v0.54.0] - 2026-07-01

### Added
- **edge-sentinel:** backfill timestamp-preservation spike (Phase 0 gate)

## [v0.53.0] - 2026-07-01

### Added
- **edge-sentinel:** VM ingest/cardinality feasibility spike (Phase 0 gate)

## [v0.52.0] - 2026-07-01

### Added
- **edge-sentinel:** throwaway VictoriaMetrics test harness (Phase 0 gate)

## [v0.51.4] - 2026-06-30

### Fixed
- **web:** kill virtualization mutation survivors + tidy edge-sentinel plans

## [v0.51.3] - 2026-06-29

### Fixed
- **terraform:** align Always-Free A1 guards to current 2 OCPU / 12 GB entitlement

## [v0.51.2] - 2026-06-28

### Fixed
- **web:** harden apiAction against empty-body error responses

## [v0.51.1] - 2026-06-28

### Fixed
- **security:** land audit-sweep findings + fuzz workflow target bug

## [v0.51.0] - 2026-06-24

### Added
- **terraform:** codify backup bucket, lifecycle + IAM policy

## [v0.50.1] - 2026-06-23

### Fixed
- **hooks:** doc-link check skips Markdown outside the repo root

## [v0.50.0] - 2026-06-19

### Added
- **agent:** add reconnect flap-guard and full-jitter backoff

## [v0.49.0] - 2026-06-19

### Added
- **ci:** persist load-test trends in VictoriaMetrics

## [v0.48.0] - 2026-06-19

### Added
- **ci:** migrate trend family to VictoriaMetrics

## [v0.47.0] - 2026-06-19

### Added
- **ci:** add benchmark trend pipeline

## [v0.46.0] - 2026-06-18

### Added
- **ci:** add VictoriaMetrics trend transport

## [v0.45.0] - 2026-06-18

### Added
- **agentapi:** 0x14 fast-path reconnect + load-test threshold

### Fixed
- **agentapi:** client-first QUIC handshake + load-test/dep fixes

## [v0.44.4] - 2026-06-12

### Fixed
- **observability:** qualify Loki transport image

## [v0.44.3] - 2026-06-12

### Fixed
- isolate cargo module snapshot logging

## [v0.44.2] - 2026-06-11

### Fixed
- **helm:** restore server env vars the compose->OKE cutover dropped

## [v0.44.1] - 2026-06-11

### Fixed
- restore post-cutover agent manifest + Loki trend pipelines; record reclaim

## [v0.44.0] - 2026-06-11

### Added
- **infra:** OKE free-tier block-volume remediation (450->200 GB)

## [v0.43.5] - 2026-06-11

### Fixed
- **monitoring:** give VictoriaMetrics a serviceaccount for kubernetes_sd

## [v0.43.4] - 2026-06-10

### Fixed
- **monitoring:** make the OKE monitoring stack fully functional (promtail logs, uptime-kuma, grafana)

## [v0.43.3] - 2026-06-10

### Fixed
- **ci:** resolve post-cutover CI failures + clear two pre-existing gauntlet breaks

## [v0.43.2] - 2026-06-06

### Fixed
- **deploy:** harden k8s CD for cutover — sharedKeys/domain in prod, non-destructive secret sync, qualified monitoring images

## [v0.43.1] - 2026-06-06

### Fixed
- **helm:** OKE cutover fixes — qualified image names + hostPort-aware strategy

## [v0.43.0] - 2026-06-06

### Added
- **infra:** wire OKE module into root terraform (Phase 13b cutover step 1)

## [v0.42.0] - 2026-06-06

### Added
- **k8s:** Phase 13b PR-E — scale-out (KEDA autoscaling + shared keys + PDB)

## [v0.41.0] - 2026-06-05

### Added
- **relay:** Phase 13b PR-D — make e2e-multiserver + cross-server load baseline

## [v0.40.1] - 2026-06-05

### Fixed
- **hooks:** deterministic commit⇒push via git native post-commit hook

## [v0.40.0] - 2026-06-05

### Added
- **relay:** Phase 13b PR-C C3b/c — degraded-mode refuse + registry_up metric + Telegram alert (ADR-023)

## [v0.39.1] - 2026-06-05

### Fixed
- **ci:** self-contained Sonar Docker fallback + nightly load-test + commit→push hooks

## [v0.39.0] - 2026-06-05

### Added
- **relay:** Phase 13b PR-C C3a — registry health + readiness drain (ADR-023)

## [v0.38.0] - 2026-06-05

### Added
- **relay:** Phase 13b PR-C C2 — cross-server WS proxy + internal listener (ADR-023)

## [v0.37.1] - 2026-06-04

### Fixed
- **ci:** cover mps OpenChannel to clear new_coverage gate + local 80.0-boundary guardrail

## [v0.37.0] - 2026-06-03

### Added
- **relay:** Phase 13b PR-C C1 — Redis Sentinel SessionRegistry + backend selection (ADR-023)

## [v0.36.2] - 2026-06-03

### Fixed
- **ci:** stop Go mutation false-timeouts dragging the nightly red

## [v0.36.1] - 2026-06-03

### Fixed
- **ci:** pin Security Audit job to go1.26.4 for govulncheck
- **ci:** unblock dev — Go 1.26.4 (stdlib CVEs) + resilient SonarCloud + /refactor for deploy/ & scripts/

## [v0.36.0] - 2026-06-02

### Added
- **k8s:** Phase 13b PR-B — OKE Helm charts + terraform + dormant CD cutover

## [v0.35.0] - 2026-06-02

### Added
- **relay:** wire InProcessRegistry live (Phase 13b PR-A) + deterministic tests (ADR-029)

### Fixed
- **ci:** restore new_coverage gate (exclude testpg, cover relay registry errors)
- **pentest:** make the gate see unstaged changes (no /precommit false-green)

## [v0.34.1] - 2026-06-01

### Fixed
- **ci:** pmat-trend JSON parse (ANSI + dual-object) + ADR plan-link guard

## [v0.34.0] - 2026-06-01

### Added
- **quality:** PMAT overlay (ADR-019/028) + Phase 13b re-eval

## [v0.33.0] - 2026-05-31

### Added
- **security:** ADR-027 adversarial pen-test gate (Semgrep) at commit + CI

### Fixed
- **ci:** pin setuptools<81 so semgrep can import pkg_resources
- **ci:** pen-test gate semgrep install — suppress upgrade notice, robust version parse

## [v0.32.1] - 2026-05-20

### Fixed
- **ci:** build-image HEAD_SHA uses github.sha to match CD's SHA source

## [v0.32.0] - 2026-05-19

### Added
- **ci,cd:** path-gate build-image.yml + digest-aware CD redeploy

## [v0.31.3] - 2026-05-19

### Fixed
- **infra:** harden bastion-session.sh — debug mode, persistent log, orphan reuse, diagnose subcommand

## [v0.31.2] - 2026-05-19

### Fixed
- **ci,agent:** path-gate release-agent.yml + agent-side sha256 precheck

## [v0.31.1] - 2026-05-19

### Fixed
- **infra:** bastion-session TTL + SSH-metadata fetch; grafana datasource UID + LogQL parse errors

## [v0.31.0] - 2026-05-19

### Added
- **infra:** OCI Bastion for operator SSH + monitoring tunnels (ADR-018)

### Changed
- **infra:** drop unused bastion + networking outputs

## [v0.30.4] - 2026-05-18

### Fixed
- **ci:** redirect GITHUB_STEP_SUMMARY for stryker step

## [v0.30.3] - 2026-05-16

### Fixed
- **ci+hooks:** hook runs full gauntlet; no marker bypass; mask tbody for stable visual regression
- **ci:** mutation push targets dev (not main); terraform-drift bisects DNS

## [v0.30.2] - 2026-05-15

### Fixed
- **ci:** drop e2e needs:[…] for parallel run; remove flaky admin-users visual baseline

## [v0.30.1] - 2026-05-15

### Fixed
- **ci:** start Postgres after checkout; rebase JSONL push with retry

## [v0.30.0] - 2026-05-15

### Added
- **protocol:** bidirectional goldens + sidecars + Go fuzzing; modularize CLAUDE.md

### Fixed
- **ci:** wrap flaky network steps in retry; mutation.yml push-to-dev

## [v0.29.0] - 2026-05-14

### Added
- **deploy:** IaC security testing pyramid — gitleaks + Checkov + Hadolint + Conftest + plan-preview

## [v0.28.0] - 2026-05-14

### Added
- **deploy:** terraform — remote state + modules + tftest + drift

### Fixed
- **deploy:** make ssh-not-world tftest portable to Terraform 1.9
- **deploy:** OCI S3-compat — disable AWS-SDK-v2 chunked-encoding upload

## [v0.27.0] - 2026-05-13

### Added
- **hooks:** activate deterministic enforcement of CLAUDE.md MANDATORY blocks (Claude Hooks PR 2)

## [v0.26.4] - 2026-05-08

### Fixed
- **deps:** bump Go toolchain to 1.26.3 and golang.org/x/net to 0.53.0

## [v0.26.3] - 2026-05-07

### Fixed
- **deps:** bump rustls-webpki to 0.103.13 (RUSTSEC-2026-0104)

## [v0.26.2] - 2026-04-20

### Fixed
- **deps:** bump pgx/v5 to 5.9.2 to patch CVE-2026-33816

## [v0.26.1] - 2026-04-16

### Fixed
- **deps:** bump rand to 0.9.3 to patch RUSTSEC-2026-0097
- **cert:** tighten test-fixture file perms to satisfy go:S2612

## [v0.26.0] - 2026-04-15

### Added
- **db:** remove SQLite backend after Postgres cutover (Phase 13a PR-6)

### Fixed
- **deps:** bump rustls-webpki to 0.103.12 to patch RUSTSEC-2026-0098/0099

## [v0.25.0] - 2026-04-12

### Added
- **dx:** add local SonarCloud analysis via Docker

## [v0.24.0] - 2026-04-12

### Added
- **deploy:** staging + production cutover preparation (Phase 13a PR-5)

### Fixed
- **deploy:** save previous tag before overwriting .env for rollback

## [v0.23.0] - 2026-04-12

### Added
- **web:** fix polling state reset, add token cleanup, improve dashboard tiles
- **deploy:** add PostgreSQL service, backups, and monitoring (Phase 13a PR-4)

### Fixed
- **web:** resolve SonarCloud issues and improve new-code coverage
- **ci:** add POSTGRES_PASSWORD to Docker Compose validation env

## [v0.22.0] - 2026-04-11

### Added
- **db:** add PostgreSQL backend alongside SQLite (Phase 13a PR-1/2)

### Changed
- **db:** inline postgres device-log SQL and drop dynamic-schema test setup
- **db:** extract duplicated test literals in postgres_test to clear SonarCloud

## [v0.21.9] - 2026-04-11

### Fixed
- **device-logs:** severity-based level filter + clear stale logs on device nav

### Changed
- **db:** inline device-log SQL literals so SonarCloud scanner accepts them
- **db:** build device-log query from SQL constants to clear SonarCloud hotspot

## [v0.21.8] - 2026-04-11

### Fixed
- **ci:** parse Rust coverage % from lcov.info, not stdout

## [v0.21.7] - 2026-04-11

### Fixed
- **docker:** apk upgrade to patch HIGH libcrypto3 CVE-2026-28390

## [v0.21.6] - 2026-04-10

### Fixed
- **ci:** exclude WSMAN protocol files from coverage analysis
- **ci:** downgrade TypeScript to 5.x for openapi-typescript compat
- **ci:** downgrade ESLint to v9, suppress MD5 hotspot in digest auth
- TypeScript 6 Uint8Array compat, SonarCloud quality gate
- narrow Uint8Array return type for TypeScript 6 compatibility

### Changed
- eliminate 12 lint suppressions and upgrade dependencies

## [v0.21.5] - 2026-04-10

### Fixed
- **ci:** resolve SonarCloud quality gate — coverage exclusions and tests
- **ci:** resolve SonarCloud quality gate failure
- handle silently swallowed errors across agent, server, and web

### Changed
- extract duplicated test fixtures in device tests

## [v0.21.4] - 2026-04-06

### Fixed
- handle 202 response in log fetching and remove refresh on retry

## [v0.21.3] - 2026-04-03

### Fixed
- normalize OS at registration and add force-refresh for device logs

## [v0.21.2] - 2026-04-03

### Fixed
- resolve CI lint failures — rustfmt and test limit mismatch
- improve log retrieval — newest-first, working filters, 300 limit

## [v0.21.1] - 2026-04-03

### Fixed
- resolve log fetch crash and upgrade button issues, add bulk upgrade

## [v0.21.0] - 2026-04-03

### Added
- redesign device detail UI with upgrade button and side-by-side logs

## [v0.20.0] - 2026-04-02

### Added
- add on-demand device log retrieval via QUIC control path

## [v0.19.1] - 2026-04-02

### Fixed
- use camelCase noDataState in Grafana alert provisioning

## [v0.19.0] - 2026-04-02

### Added
- add /observe skill for autonomous diagnostics

## [v0.18.2] - 2026-04-01

### Fixed
- sync go.sum with go.mod for modernc.org/sqlite v1.47.0

## [v0.18.1] - 2026-03-31

### Fixed
- tune k6 scenarios to stay within per-IP rate limit

## [v0.18.0] - 2026-03-31

### Added
- add agent restart and on-demand hardware inventory

## [v0.17.1] - 2026-03-30

### Fixed
- hide Desktop and Chat tabs when capabilities are unknown

## [v0.17.0] - 2026-03-27

### Added
- system tray with IPC for desktop agent management

### Fixed
- exclude tray crate from headless CI, add dedicated desktop CI job

## [v0.16.1] - 2026-03-27

### Fixed
- correct agent online/offline status misreporting across all layers

## [v0.16.0] - 2026-03-26

### Added
- X11 screen capture with JPEG encoding + chat echo

## [v0.15.8] - 2026-03-26

### Fixed
- show error toast when Start Session fails silently

## [v0.15.7] - 2026-03-26

### Fixed
- restore SELinux context after OTA binary replacement

## [v0.15.6] - 2026-03-26

### Fixed
- probe display sockets instead of checking existence

### Changed
- extract probe_socket helper and deduplicate display tests

## [v0.15.5] - 2026-03-26

### Fixed
- agent startup ordering and version auto-sync from git tags

## [v0.15.4] - 2026-03-26

### Fixed
- detect display server via socket probing for systemd services

## [v0.15.3] - 2026-03-26

### Fixed
- stop capture loop after 3 consecutive failures instead of spamming

## [v0.15.2] - 2026-03-26

### Fixed
- remove invalid -rule flag from VictoriaMetrics single-node config

## [v0.15.1] - 2026-03-26

### Fixed
- OTA push returns 0 due to OS/arch mismatch between agent and manifest

## [v0.15.0] - 2026-03-26

### Added
- file manager view/download and capability-based session tabs

## [v0.14.4] - 2026-03-25

### Fixed
- npm deps bump

## [v0.14.3] - 2026-03-25

### Fixed
- tie agent version to git release tags instead of stale Cargo.toml

## [v0.14.2] - 2026-03-25

### Fixed
- use sudo for cosign install on VPS

## [v0.14.1] - 2026-03-25

### Fixed
- verify cosign signature before stopping containers and auto-install cosign on VPS

## [v0.14.0] - 2026-03-25

### Added
- add cosign keyless signing, SBOM attestation, and deploy-time verification

## [v0.13.14] - 2026-03-25

### Fixed
- add artifact retention policy and replace silent error discards

## [v0.13.13] - 2026-03-25

### Fixed
- use latest tag for Trivy image scan instead of full SHA

## [v0.13.12] - 2026-03-25

### Fixed
- replace all silent skip patterns in Makefile with loud failures

## [v0.13.11] - 2026-03-25

### Fixed
- resolve CI codegen check path issue and remove silent skip patterns
- use direct oapi-codegen invocation for codegen sync check
- install oapi-codegen in CI before codegen sync check

## [v0.13.10] - 2026-03-24

### Fixed
- wrap monitoring deploy comment to satisfy yamllint line-length rule
- resolve Grafana provisioning crashes and datasource UID mismatches

## [v0.13.9] - 2026-03-24

### Fixed
- quote Telegram chat ID to prevent Grafana provisioning crash

## [v0.13.8] - 2026-03-23

### Fixed
- implement http.Hijacker on metrics statusWriter to unblock WebSocket upgrades

## [v0.13.7] - 2026-03-23

### Fixed
- relay pipe context, ungrouped device access, and device listing query

## [v0.13.6] - 2026-03-23

### Fixed
- preserve WebSocket message boundaries in relay

## [v0.13.5] - 2026-03-23

### Fixed
- replace ReadTimeout/WriteTimeout with ReadHeaderTimeout for WebSocket compatibility

## [v0.13.4] - 2026-03-23

### Fixed
- pass Telegram env vars to Grafana container

## [v0.13.3] - 2026-03-23

### Fixed
- use container name for Uptime Kuma in Caddyfile

## [v0.13.2] - 2026-03-23

### Fixed
- skip /metrics smoke test in staging and production

## [v0.13.1] - 2026-03-23

### Fixed
- inject .env.monitoring via CD pipeline from GitHub Secrets

## [v0.13.0] - 2026-03-23

### Added
- add Phase D monitoring & observability stack

## [v0.12.0] - 2026-03-21

### Added
- add frontend performance monitoring to CI/CD pipeline

### Fixed
- upgrade rustls-webpki 0.103.9 → 0.103.10 (RUSTSEC-2026-0049)

## [v0.11.2] - 2026-03-20

### Fixed
- add "Add Device" button to /devices top-right corner

## [v0.11.1] - 2026-03-20

### Fixed
- UI bugs — duplicate buttons, search placeholder, device version sync

## [v0.11.0] - 2026-03-20

### Added
- complete agent auto-update system (Phase 14, ADR-121)

### Changed
- extract constants and helpers in Phase 14 code

## [v0.10.0] - 2026-03-20

### Added
- add frontend-audit skill

## [v0.9.5] - 2026-03-20

### Fixed
- UI bugs, terminal hotkeys, file error feedback, agent uninstall

## [v0.9.4] - 2026-03-20

### Fixed
- use server CA for QUIC load test mTLS handshake

## [v0.9.3] - 2026-03-20

### Fixed
- tune UDP buffers for QUIC load test (100+ agents)
- session permissions, agent deregistration, UI cleanup

## [v0.9.2] - 2026-03-19

### Fixed
- only increment enrollment token use count when CSR is signed

## [v0.9.1] - 2026-03-19

### Fixed
- resolve all 53 code scanning alerts across Go, TypeScript, Rust, JS

## [v0.9.0] - 2026-03-19

### Added
- close web UI API coverage gaps, fix 4 agent/session bugs

## [v0.8.1] - 2026-03-18

### Fixed
- use X-Forwarded-Proto for relay URL scheme behind reverse proxy

## [v0.8.0] - 2026-03-18

### Added
- list all devices without requiring group_id filter

## [v0.7.7] - 2026-03-18

### Fixed
- nullable device group_id to prevent FK violation on agent registration

## [v0.7.6] - 2026-03-18

### Fixed
- CSR-based agent enrollment to resolve TLS error 48 (unknown_ca)

## [v0.7.5] - 2026-03-18

### Fixed
- use actual hostname as QUIC TLS SNI, fix E2E strict mode violation

### Changed
- DRY helpers, module splits, and polish across all layers

## [v0.7.4] - 2026-03-18

### Fixed
- add missing global-teardown.ts for Playwright config
- use cross for aarch64 musl build instead of musl.cc download

## [v0.7.3] - 2026-03-18

### Fixed
- E2E test improvements and public update manifests endpoint
- build agent with musl for static binaries (no glibc dependency)

## [v0.7.2] - 2026-03-18

### Fixed
- include QUIC hostname in server TLS certificate SANs

## [v0.7.1] - 2026-03-18

### Fixed
- resolve DNS hostnames in agent QUIC connection

## [v0.7.0] - 2026-03-17

### Added
- add OPENGATE_QUIC_HOST to override QUIC address in enrollment

## [v0.6.1] - 2026-03-17

### Fixed
- inject OPENGATE_GITHUB_REPO into install script

## [v0.6.0] - 2026-03-17

### Added
- add Playwright E2E tests to precommit checklist

## [v0.5.0] - 2026-03-17

### Added
- inject server URL into install script from request headers

## [v0.4.3] - 2026-03-17

### Fixed
- remove service worker precache that served stale index.html

## [v0.4.2] - 2026-03-17

### Fixed
- global-setup reads baseURL from Playwright config

## [v0.4.1] - 2026-03-17

### Fixed
- rename unused loop variable to _ to satisfy shellcheck SC2034
- reset staging DB before e2e tests for fresh admin bootstrap

## [v0.4.0] - 2026-03-17

### Added
- add security groups and permissions management

### Fixed
- use exact match for Management sidebar text in e2e test
- correct e2e selectors for admin heading and System badge
- block SPA rendering until auth hydration completes
- e2e admin page tests race condition and last-admin test
- e2e admin tests fail when not first user in shared DB

## [v0.3.0] - 2026-03-17

### Added
- add enrollment token management to setup page

## [v0.2.0] - 2026-03-17

### Added
- auto-sync agent manifests from GitHub Releases on startup

## [v0.1.1] - 2026-03-17

### Fixed
- switch tokio-tungstenite from native-tls to rustls

## [v0.1.0] - 2026-03-16

### Added
- add QUIC control loop, auto-tag CI job, and changelog
