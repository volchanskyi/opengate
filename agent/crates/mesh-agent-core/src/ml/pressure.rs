...
#[cfg(test)]
mod tests {
    use super::{own_cgroup, parse_avg60, FULL, SOME};
    use std::fs;
    use std::path::Path;
    ...
    /// malformed entry can never point the reader at an arbitrary file. The
    /// screen runs on the text, before it becomes a path at all.
    #[test]
