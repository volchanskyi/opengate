...
| `instance_ocpus` | number | OCPUs for the shape. Always Free cap is 2 OCPUs total per tenancy. |
| `instance_memory_gb` | number | Memory in GB. Always Free cap is 12 GB total per tenancy. |
...
- [`tests/free_tier.tftest.hcl`](tests/free_tier.tftest.hcl) — Always Free shape and sizing limits (shape pinned to A1.Flex, ≤2 OCPUs, ≤12 GB memory, ≤200 GB boot volume). Runs against a mock provider — no OCI creds required.
