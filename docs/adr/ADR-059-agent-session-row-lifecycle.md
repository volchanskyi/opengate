...
**1. Unpaired release, for what the process can see.** `Relay.Unregister` ends a
session that never reached the pipe: it drops the map entry, decrements the
active count, releases the registry record, closes any conn still waiting on the
missing peer, and fires `OnSessionEnd` so the row goes with it. The relay
WebSocket handler defers it unconditionally after a successful register. A
session that reached the pipe is left alone — that teardown belongs to the pipe,
and a second release would double-count — and a `released` flag under the session
lock keeps a peer registering concurrently from starting a pipe on a session the
release already claimed.

**2. A relay-keyed sweep, for what it cannot.** A periodic job deletes every row
...
- `session.Repository` gains `DeleteStale(ctx, cutoff, keep)`, which carries its
  own admin tenant scope so a background caller needs none. RLS widens the
  admin scope to every organization, which is what makes the sweep fleet-wide.
...
- `OnSessionEnd` tolerates `ErrSessionNotFound`: a row the sweep already
  collected is the expected race, not a failure.
- A session row can outlive its connection by up to the grace period when the
  process that owned it died. Bounded and self-healing, where it was unbounded.
