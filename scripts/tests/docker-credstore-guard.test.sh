...
if [ "$RC" = 0 ] && [ "$OUT" = "$TMP/dcfg" ]; then pass "no config.json: src dir unchanged"
else fail "no config.json (rc=$RC out=$OUT want=$TMP/dcfg)"; fi
rm -rf "$TMP"

# 2. config.json without credsStore → src dir unchanged.
TMP="$(mktemp -d)"; mkdir -p "$TMP/dcfg"; printf '{"proxies":{}}' > "$TMP/dcfg/config.json"
run_guard "$TMP/dcfg" "$TMP/cache"
if [ "$RC" = 0 ] && [ "$OUT" = "$TMP/dcfg" ]; then pass "no credsStore: src dir unchanged"
else fail "no credsStore (rc=$RC out=$OUT)"; fi
rm -rf "$TMP"

# 3. Working credsStore helper → src dir unchanged (helper probe passes).
TMP="$(mktemp -d)"; mkdir -p "$TMP/dcfg" "$TMP/bin"
