...
mapfile -t PULL_JOBS < <(
  for workflow in "$WORKFLOWS"/*.yml; do
    awk '
      ...
      # e2e-stack-up.sh is a pull line because it is a compose bring-up: the
      # job reaches the registry through the script rather than in the step, and
      # a detector that only reads the step would stop seeing the e2e job.
      function is_pull_line(line) {
        return line ~ /docker[[:space:]]+pull[[:space:]]/ || line ~ /docker[[:space:]]+run[[:space:]]/ || line ~ /docker[[:space:]]+compose.*[[:space:]]up([[:space:]]|$)/ || line ~ /e2e-stack-up\.sh/ || line ~ /^      image:[[:space:]]/ || line ~ /^        image:[[:space:]]/
      }
