...
func TestCreateSessionAgentNotConnected(t *testing.T) {
	...
	d := testutil.SeedDevice(t, ctx, srv.store, site.ID)
...
func newCreateSessionEnv(t *testing.T) createSessionEnv {
	...
	ac := agentapi.NewAgentConn(agentapi.AgentConnConfig{DeviceID: d.ID, SiteID: site.ID, Stream: &agentStream, Devices: testutil.NewTestDevices(t, store), Hardware: testutil.NewTestHardware(t, store), DeviceUpdates: testutil.NewTestDeviceUpdates(t, store), Logger: logger})
