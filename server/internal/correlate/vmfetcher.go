...
var reservedLabels = map[string]bool{"tenant_id": true, "device_id": true, "__name__": true}
...
type vmExporter interface {
	Export(ctx context.Context, tenantID uuid.UUID, selector string, start, end time.Time) ([]telemetry.ExportedSeries, error)
}
...
// The selector deliberately omits tenant_id; telemetry.VMClient injects the
// authoritative tenant matcher, so one tenant can never read another's series.
func (f *VMFetcher) Fetch(ctx context.Context, tenantID, deviceID uuid.UUID, start, end time.Time) ([]Series, error) {
	selector := fmt.Sprintf(`{device_id=%q}`, deviceID.String())
	exported, err := f.vm.Export(ctx, tenantID, selector, start, end)
	if err != nil {
		return nil, err
	}
