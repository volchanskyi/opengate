...
func TestInstrumentedDevices_AllMethods(t *testing.T) {
	...
	t.Run("success paths", func(t *testing.T) {
		...
		require.NoError(t, err)
		_, err = r.List(ctx, device.Filter{SiteID: uuid.New()})
		require.NoError(t, err)
		_, err = r.List(ctx, device.Filter{})
		require.NoError(t, err)
		_, err = r.GetByAMTUUID(ctx, uuid.New())
		require.NoError(t, err)
		require.NoError(t, r.Delete(ctx, uuid.New()))
		require.NoError(t, r.UpdateSite(ctx, uuid.New(), uuid.New()))
	...
	t.Run("error paths", func(t *testing.T) {
		...
		assert.Error(t, r.UpdateSite(ctx, uuid.New(), uuid.New()))
