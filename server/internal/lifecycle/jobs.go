...
func (s *JobStore) CreateJob(ctx context.Context, job *PurgeJob) error {
	_, err := s.db.ExecContext(ctx,
		`INSERT INTO purge_jobs (id, tenant_id, device_id, scope, state, requested_by)
		 VALUES ($1, $2, $3, $4, $5, $6)`,
		job.ID, job.TenantID, nullableUUID(job.DeviceID), string(job.Scope), string(job.State), nullableUUID(job.RequestedBy))
	if err != nil {
...
// LatestForTenant returns the most recent purge job for a tenant, or nil when the tenant
// has never been purged. The status API uses it to report progress.
func (s *JobStore) LatestForTenant(ctx context.Context, tenantID uuid.UUID) (*PurgeJob, error) {
	row := s.db.QueryRowContext(ctx, latestPurgeJobForTenantSQL, tenantID)
	job, err := scanJob(row)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, nil
	}
...
const (
	purgeJobColumns = `SELECT id, tenant_id, device_id, scope, state, vm_deleted, object_deleted,
	...
	latestPurgeJobForTenantSQL = purgeJobColumns + ` WHERE tenant_id = $1 ORDER BY created_at DESC LIMIT 1`
)
...
func scanJob(row rowScanner) (*PurgeJob, error) {
	...
	if err := row.Scan(&job.ID, &job.TenantID, &device, &scope, &state, &job.VMDeleted,
