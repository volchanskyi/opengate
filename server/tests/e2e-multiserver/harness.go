// Package main implements the multiserver e2e harness: a
// host-side wire driver that seeds sessions in the shared Postgres and drives raw
// relay WebSockets across two server replicas to prove cross-server proxy and
// Redis-loss degraded-mode behavior. It is invoked by
